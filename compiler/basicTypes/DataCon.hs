...
data HsBang
  ...
  = HsSrcBang    -- What the user wrote in the source code
       ...
       (Maybe SrcUnpackedness) -- Nothing, no unpack pragma
                               -- Just _, UNPACK or NOUNPACK specified
       (Maybe SrcStrictness)   -- Nothing, no strictness annotation
                               -- Just _, '!' or '~' specified
  ...
  | HsUnpack -- ^ Definitie commitment: Strict and unpacked field
...
instance Outputable HsBang where
    ppr (HsSrcBang _ prag mark) = pp_unpk prag <+> pp_mark mark
    ppr HsLazy                  = empty
...
pp_mark :: Maybe SrcStrictness -> SDoc
pp_mark Nothing      = empty
pp_mark (Just mark)  = ppr mark

pp_unpk :: Maybe SrcUnpackedness -> SDoc
pp_unpk Nothing      = empty
pp_unpk (Just unpk)  = ppr unpk
...
isBanged _ (HsSrcBang _ _ (Just SrcStrict)) = True
