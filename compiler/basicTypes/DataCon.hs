...
-- See Note [Bangs on data constructor arguments]
data HsBang
  = HsNoBang     -- Equivalent to (HsSrcBang Nothing (Just False))
  ...
  | HsSrcBang    -- What the user wrote in the source code
       ...
       (Maybe Bool)       -- Just True    {-# UNPACK #-}
                          -- Just False   {-# NOUNPACK #-}
                          -- Nothing      no pragma
       (Maybe Bool)       -- Just True, '!' specified
                          -- Just False, '~' specified
                          -- Nothing, nothing specified
       -- (HsSrcBang (Just True) (Just False)) makes no sense
       -- We emit a warning (in checkValidDataCon) and treat it
       -- just like (HsSrcBang Nothing (Just False))
...
pp_mark :: Maybe Bool -> SDoc
pp_mark Nothing      = empty
pp_mark (Just True)  = char '!'
pp_mark (Just False) = char '~'
...
isBanged (HsSrcBang _ _ Nothing)     = False
isBanged (HsSrcBang _ _ (Just mark)) = mark
